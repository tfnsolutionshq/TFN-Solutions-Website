        ...
        >
          {[...categories, ...categories].map((category, index) => (
            <div key={index} className="inline-flex items-center">
              <span className={`text-5xl font-semibold mx-4 ${index % 2 === 0 ? 'text-black' : 'text-gray-400'}`}>
                {category}
              </span>
              <span className={`text-5xl mx-4 ${index % 2 === 0 ? 'text-black' : 'text-gray-400'}`}>-</span>
            </div>
          ))}
          <div className="inline-block w-32 h-1 bg-black"></div>
        </motion.div>
      </div>
